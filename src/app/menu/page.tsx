...
export default async function MenuPage() {
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden font-sans">
      ...
      {/* ── HERO BANNER ─────────────────────────────────────── */}
      <section className="bg-gradient-to-r from-black to-[#21c55e] pt-20 sm:pt-28 pb-10 sm:pb-16 px-5 sm:px-8 text-center relative overflow-hidden font-sans">
        <div className="relative z-10">
          <div
            className="inline-flex items-center gap-2 bg-[#C8963E]/15 border border-[#C8963E]/35
                          px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-semibold text-[#C8963E]
                          tracking-widest uppercase mb-4 sm:mb-5"
          >
            ✦ Full Menu
          </div>
          <h1 className="text-[clamp(2rem,6vw,4rem)] font-bold text-white leading-[1.08] mb-3 sm:mb-4 font-sans">
            Made Fresh,{" "}
            <em className="not-italic text-[#e9bd6b]">Served with Soul.</em>
          </h1>
          ...
          {/* Category quick-jump links — horizontal scroll on mobile */}
            ...
            {Object.entries(grouped).map(([cat]) => {
              ...
              return (
                <a
                  key={cat}
                  href={`#cat-${cat}`}
                  className="bg-white/10 border border-white/10 text-white/80 text-xs
                             px-3 sm:px-4 py-1.5 rounded-full hover:bg-[#C8963E]/20
      ...
      {/* ── NOTICE BANNER ───────────────────────────────────── */}
      <div className="bg-[#ddd]/50 border-y border-[#C8963E]/20 px-4 sm:px-6 py-2.5 sm:py-3 text-center">
        <p className="text-xs sm:text-sm text-[#085208] leading-snug">
      ...
      {/* ── MENU SECTIONS ───────────────────────────────────── */}
      <main className="px-4 sm:px-8 md:px-16 py-10 sm:py-16 max-w-7xl mx-auto">
        {Object.entries(grouped).map(([cat, catItems]) => {
          ...
          return (
            ...
            >
              {/* Category header */}
              <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 pb-3 sm:pb-4 border-b border-[#C8963E]/15">
                <span className="text-2xl sm:text-3xl">{meta.icon}</span>
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg sm:text-2xl font-bold text-[#065109] font-sans">
                    {meta.label}
                  </h2>
                  <p className="text-xs sm:text-sm text-[#6B5540] mt-0.5 hidden sm:block">
                    {meta.desc}
                  </p>
                </div>
                <span
                  className="ml-auto text-xs font-semibold text-[#C8963E] bg-[#C8963E]/10
                                  px-2.5 sm:px-3 py-1 rounded-full whitespace-nowrap flex-shrink-0"
                >
                  {catItems.length} item{catItems.length !== 1 ? "s" : ""}
                </span>
              </div>

              {/* Dish grid — 2 cols on mobile, 3 on sm, 4 on xl */}
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
                {catItems.map((item) => (
                  <div
                    key={item.id}
                    className="bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-[#C8963E]/10 shadow-md
                               hover:shadow-[0_12px_40px_rgba(26,15,5,0.08)] transition-all
                               hover:-translate-y-0.5 group flex flex-col"
                  >
                    {/* Image */}
                    <div className="h-28 sm:h-32 w-full relative overflow-hidden bg-[#f5e6cf]">
                      {item.imageUrl ? (
                        <img
                          src={item.imageUrl}
                          alt={item.name}
                      ...
                      {item.isFeatured && (
                        <span className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 bg-[#C8963E] text-white text-[8px] sm:text-[9px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full tracking-wide">
                          ★ POPULAR
                        </span>
                      )}

                      {item.isSpicy && (
                        <span className="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 text-xs sm:text-sm">
                          🌶️
                        </span>
                      )}
                    </div>
                    ...
                    <div className="p-3 sm:p-4 flex flex-col flex-1">
                      <h3 className="font-bold text-[#1A0F05] text-sm sm:text-base mb-1 leading-tight line-clamp-2">
                        {item.name}
                      </h3>
                      {item.description && (
                        <p className="text-[11px] sm:text-xs text-[#6B5540] leading-relaxed mb-2 sm:mb-3 line-clamp-2 flex-1 hidden sm:block">
                          {item.description}
                        </p>
                      )}
                          ...
                          {item.tags.slice(0, 2).map((tag) => (
                            <span
                              ...
                              className="text-[8px] sm:text-[9px] font-semibold uppercase tracking-wide
                                         bg-[#C8963E]/08 text-[#C8963E] px-1.5 sm:px-2 py-0.5 rounded-full border border-[#C8963E]/15"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      <div
                        className="flex items-center justify-between mt-auto pt-2 sm:pt-3 font-sans
                      ...
                      >
                        ...
                        <Link
                          href="/order"
                          className="text-[10px] sm:text-xs bg-[#0a5109] text-white px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-md
                                     hover:opacity-100 opacity-90 transition-colors font-medium whitespace-nowrap"
                        >
                          Add to Order
                        </Link>
                      </div>
        ...
        {Object.keys(grouped).length === 0 && (
          <div className="text-center py-16 sm:py-24">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-[#085208] mb-2">
              Menu Coming Soon
            </h3>
      ...
      {/* ── CTA STRIP ───────────────────────────────────────── */}
      <section
        className="mx-4 sm:mx-8 md:mx-16 mb-10 sm:mb-16 bg-gradient-to-br from-green-500 to-black
                      rounded-[20px] sm:rounded-[24px] px-6 sm:px-8 md:px-14
                      py-10 sm:py-12 flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-6
        ...
        <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0 relative z-10 w-full md:w-auto">
          <Link
            ...
            className="bg-green-400 text-white font-semibold px-6 py-3 rounded-full
                       hover:bg-white hover:text-green-500 transition-all text-sm text-center"
          >
            Plan an Event →
          </Link>
          <Link
            href="/order"
            className="bg-white/10 border border-white text-white font-medium
